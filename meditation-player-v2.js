(function() {
    ...
    window.likeMedTrack = function(e, id) {
        ...
        var heart = btn.querySelector('.med-heart');
        if (heart) {
            heart.style.fill   = liked ? 'none'    : 'rgba(0,200,220,0.9)';
            heart.style.stroke = liked ? 'currentColor' : 'rgba(0,200,220,0.9)';
        }
        ...
        MEDITATIONS.forEach(function(med) {
            ...
            div.innerHTML =
                ...
                '<div style="display:flex;align-items:center;gap:10px;flex-shrink:0;">' +
                    ...
                    '<button onclick="likeMedTrack(event,' + med.id + ')" title="J\'aime" style="background:none;border:none;cursor:pointer;color:rgba(255,255,255,0.45);transition:color 0.2s,transform 0.2s;display:flex;padding:3px;" onmouseover="this.style.transform=\'scale(1.15)\'" onmouseout="this.style.transform=\'\'">' +
                        '<svg class="med-heart" viewBox="0 0 24 24" width="17" height="17" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg>' +
                    '</button>' +
                    '<button onclick="shareMedTrack(event,' + med.id + ')" title="Partager" style="background:none;border:none;cursor:pointer;color:rgba(255,255,255,0.45);transition:color 0.2s,transform 0.2s;display:flex;padding:3px;" onmouseover="this.style.transform=\'scale(1.15)\'" onmouseout="this.style.transform=\'\'">' +
                    ...
                    '</button>' +
                    '<button class="play-btn med-play-btn" onclick="toggleMedTrack(' + med.id + ');event.stopPropagation()" aria-label="Lecture" style="margin-left:4px;">' +
